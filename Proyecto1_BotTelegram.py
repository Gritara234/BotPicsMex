...
APPOINTMENT_EMAIL = os.getenv('APPOINTMENT_EMAIL')  
...
FAQS = {
    ...
    "¿Tienen algún tipo de promoción especial en tus servicios?": "Por el momento no manejamos promociones.",
    "Si tengo que cancelar o reprogramar mi cita, ¿Qué tengo que hacer? ": "Si le es necesario cancelar su cita le pedimos que se comunique con nosotros al menos 48 horas antes de la fecha acordada."
}
...
SOCIAL_MEDIA = {
    "Instagram": ("@pics_mex_social", "https://www.instagram.com/pics_mex_social/"),
    "Facebook": ("Sociales Pics", "https://www.facebook.com/socialespics/")
}
...
SERVICES = ["Fotografía de Graduación", "Fotografía de Boda", "Fotografía de Bautizos", "Fotografía de XV Años"]
    ...
    try:
        if query.data == 'prices':
            ...
            if 'location_message_id' in context.user_data:
                try:
                    await context.bot.delete_message(chat_id=query.message.chat_id, message_id=context.user_data['location_message_id'])
                    del context.user_data['location_message_id']
                except Exception as e:
                    logger.error(f"Error deleting location message: {str(e)}")
...
async def send_sample_photos(update: Update, context) -> None:
    ...
    await context.bot.send_message(chat_id=chat_id, text="Aquí tienes algunas de nuestras fotos de muestra:", reply_markup=reply_markup)
